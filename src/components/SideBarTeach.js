...
const SideBarTeach = () => {
  ...
  return (
    <div className="justify-evenly">
      <div>
        <div className={`bg-blue h-screen p-5 text-white pt-8 w-72 relative duration-300 ${open ? "w-72" : "w-20"}`}>
          <BsArrowLeftShort className={`bg-white text-blue cursor-pointer text-3xl rounded-full absolute -right-3 top-9 border border-blue ${!open && "rotate-180"}`} onClick={() => setOpen(!open)} />
          ...
          <div className="pt-2">
            <li className="text-gray-300 text-sm flex items-center gap-x-4 p-2 hover:bg-[#71bbd4] rounded-md mt-2">
              ...
              <span className={`text-base font-bold flex-1 ${!open && "hidden" }`}>Assignments</span>
            </li>
          </div>
          <div className="pt-2">
            <li className="text-gray-300 text-sm flex items-center gap-x-4 p-2 hover:bg-[#71bbd4] rounded-md mt-2">
